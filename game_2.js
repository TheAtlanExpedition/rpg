...
function createGame2State(level = 1) {
  ...
  for (let i = 0; i < rooms.length - 1; i++) {
    ...
    for (let x = Math.min(firstX, secondX); x <= Math.max(firstX, secondX); x++) {
      map[firstY][x] = FLOOR;
    }
    for (let y = Math.min(firstY, secondY); y <= Math.max(firstY, secondY); y++) {
      map[y][secondX] = FLOOR;
    }
  }
...
function movePlayer(dx, dy) {
  if (!game2Running || !game2State || game2State.gameOver) return;
  ...
  if (item) {
    game2State.items = game2State.items.filter((i) => i.id !== item.id);
    if (item.type === "scroll") {
...
function attackEnemy(enemyId) {
  ...
  const enemy = game2State.enemies.find((e) => e.id === enemyId);
  if (!enemy) return;
  ...
  if (enemy.hp <= 0) {
    game2State.enemies = game2State.enemies.filter((e) => e.id !== enemyId);
...
function castSpell() {
  ...
  game2State.scrolls--;
  const direction = game2State.player.x < MAP_WIDTH / 2 ? 1 : -1;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    hitEnemy = game2State.enemies.find(
      (e) => e.x === projectileX && e.y === game2State.player.y
    );
    if (hitEnemy) break;
  }
  ...
  if (hitEnemy) {
    hitEnemy.hp -= 20;
    game2State.message = "Magic bolt exploded near the monster!";
    if (hitEnemy.hp <= 0) {
      game2State.enemies = game2State.enemies.filter((e) => e !== hitEnemy);
    }
  } else {
...
function moveEnemies() {
  if (!game2State || game2State.gameOver) return;
  ...
  for (const enemy of game2State.enemies) {
    ...
    const possibleMoves = [
      { x: enemy.x + Math.sign(game2State.player.x - enemy.x), y: enemy.y },
      { x: enemy.x, y: enemy.y + Math.sign(game2State.player.y - enemy.y) }
    ];

    for (const move of possibleMoves) {
      const validMove =
        move.x >= 0 && move.x < MAP_WIDTH &&
        move.y >= 0 && move.y < MAP_HEIGHT &&
        game2State.map[move.y][move.x] === FLOOR &&
        !(move.x === game2State.player.x && move.y === game2State.player.y);
...
function renderGame2() {
  if (!game2State) return;

  game2Context.clearRect(0, 0, game2Canvas.width, game2Canvas.height);

  for (let y = 0; y < MAP_HEIGHT; y++) {
    for (let x = 0; x < MAP_WIDTH; x++) {
      game2Context.fillStyle = game2State.map[y][x] === WALL ? "#111827" : "#374151";
      game2Context.fillRect(x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE);
      game2Context.strokeStyle = "#1f2937";
      game2Context.strokeRect(x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE);
    }
  }
  ...
  drawText(game2State.player.symbol, game2State.player.x, game2State.player.y);
  updateGameInfo();
}

function updateGameInfo() {
  if (!gameInfo || !game2State) return;
...
function drawGame2PoweredOffScreen() {
  ...
  game2Context.fillRect(0, 0, game2Canvas.width, game2Canvas.height);
  game2Context.fillStyle = "#9ca3af";
  game2Context.font = "24px Arial";
  game2Context.textAlign = "center";
  game2Context.textBaseline = "middle";
  game2Context.fillText("Game 2 is powered off", game2Canvas.width / 2, game2Canvas.height / 2);
}
